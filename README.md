...
2. Edit `plugins/PurrtechQuest/config.yml` if the defaults don't fit (storage backend, default locale, the
   guide item, etc. — see [Configuration](#configuration) below).
...
| --- | --- |
| `/quest` or `/quest gui` | Opens the GUI (guide overview + quest list, or straight to the quest list if the guide is disabled). |
...
| `/questadmin category <name>` | Opens the editor for a quest category's description and which parts of its display are shown. |
...
- **`guide.enabled`** — on/off switch for the "guide" item in `/quest gui` (overall progress + current quest
  breakdown). Its wording and styling live in `lang/<locale>.yml` under the `guide-*` keys, fully editable;
  this is just whether it shows up at all.
- **`debug`** — extra logging.
